        ...
        children: [
          ...
          AddSourceTile(
            ...
            onTap: () async {
              ...
              if (context.mounted) {
                Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => LocalBrowserPage(store: store, source: source)));
              }
            },
          ),
          ...
          AddSourceTile(
            ...
            title: 'WebDAV',
            onTap: () => Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (_) => WebdavSourceFormPage(store: store))),
          ),
        ],
      ),
    );
...
class _WebdavSourceFormPageState extends State<WebdavSourceFormPage> {
  ...
  Future<void> save() async {
    ...
    try {
      ...
      if (!mounted) return;
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => WebdavBrowserPage(store: widget.store, source: source)));
    } catch (e) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          TextField(controller: baseUrl, decoration: const InputDecoration(labelText: '服务器地址，例如 https://host/dav')),
          ...
          TextField(controller: directory, decoration: const InputDecoration(labelText: '目录路径，例如 /Movies/')),
...
class WebdavBrowserPage extends StatefulWidget {
  const WebdavBrowserPage({required this.store, required this.source, super.key});
...
class _WebdavBrowserPageState extends State<WebdavBrowserPage> {
  ...
  MediaSourceConfig get source => widget.store.sources.firstWhere((value) => value.id == widget.source.id, orElse: () => widget.source);
  ...
  Future<void> addEntry(WebdavEntry entry) async {
    ...
    try {
      ...
      if (mounted) showSnack(context, entry.isDir ? '已添加文件夹，视频会显示在首页' : '已添加视频到首页');
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(path == '/' ? widget.source.name : path),
        actions: [IconButton(tooltip: '刷新', onPressed: refresh, icon: const Icon(Icons.refresh))],
      ),
      body: FutureBuilder<List<WebdavEntry>>(
        future: future,
        builder: (context, snapshot) {
          if (snapshot.connectionState != ConnectionState.done) return const Center(child: CircularProgressIndicator());
          if (snapshot.hasError) return ErrorView(message: '${snapshot.error}', onRetry: refresh);
          ...
          if (entries.isEmpty) {
            return EmptyState(
              ...
              action: OutlinedButton(onPressed: refresh, child: const Text('重新加载')),
            );
          }
          return ListView(
            children: [
              if (path != widget.source.directory)
                ListTile(
                  ...
                  onTap: () => refresh(parentPath(path)),
                ),
              for (final entry in entries)
                ListTile(
                  leading: Icon(entry.isDir ? Icons.folder : Icons.movie_outlined),
                  title: Text(entry.name, maxLines: 1, overflow: TextOverflow.ellipsis),
                  subtitle: Text(entry.isDir ? '文件夹' : readableBytes(entry.size)),
                  trailing: isSelected(entry)
                      ? IconButton(
                          tooltip: entry.isDir ? '取消此文件夹' : '取消此视频',
                          onPressed: adding ? null : () => removeEntry(entry),
                          icon: const Icon(Icons.check_circle, color: Color(0xFF2E7AF6)),
                        )
                      : IconButton(
                          tooltip: entry.isDir ? '添加此文件夹' : '添加此视频',
                          onPressed: adding || (!entry.isDir && !isVideoName(entry.name)) ? null : () => addEntry(entry),
                  ...
                  onTap: () {
                    if (entry.isDir) {
                      ...
                      openPlayer(context, widget.store, MediaItem.webdav(source: widget.source, entry: entry));
                    }
                  },
                ),
            ],
